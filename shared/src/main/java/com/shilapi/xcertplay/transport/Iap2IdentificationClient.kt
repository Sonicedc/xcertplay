...
/** Required identity for the minimal wired iAP2 identification exchange. */
...
sealed class Iap2IdentificationException(message: String) : IOException(message) {
    class Rejected(parameterIds: Set<Int>) : Iap2IdentificationException(
        "iAP2 identification rejected parameters ${parameterIds.sorted().joinToString(prefix = "[", postfix = "]") { "0x${it.toString(16).padStart(4, '0')}" }}; " +
            "this minimal wired profile has no optional components to remove",
    ) {
...
/**
 * Synchronous accessory-side wired identification over an already owned CSM channel.
 *
 * This is intentionally only identification: it neither invokes MFi nor itself starts any
 * CarPlay, subscription, power, media, or UI service.  The advertised messages are the exact
 * wired-control messages implemented by [Iap2WiredControlClient].
 */
class Iap2IdentificationClient(private val channel: Iap2CsmChannel) {
    ...
    companion object {
        ...
        /** Builds the smallest honest LIVI-compatible wired IdentificationInformation message. */
        fun identificationInformation(config: Iap2IdentificationConfig): CsmFrame {
            ...
            val payload = Iap2CsmParameters.encode(
                    listOf(
                        ...
                        Iap2CsmParameter(6, u16List(MESSAGES_SENT_BY_ACCESSORY)),
                        Iap2CsmParameter(7, u16List(MESSAGES_RECEIVED_FROM_PHONE)),
                        Iap2CsmParameter(8, byteArrayOf(2)),
                        ...
                        Iap2CsmParameter(16, usbHostTransport),
                    ),
            )
